...
function mineEvents:onEvent(event)
     if event and event.id then
        ...
        if (event.id == world.event.S_EVENT_SHOT and event.initiator and event.weapon) then
            if event.weapon:isExist() and (string.find(event.weapon:getTypeName(), 'BDU') or event.weapon:getTypeName() == 'FAB_50' or event.weapon:getTypeName() == 'AN_M30A1' or event.weapon:getTypeName() == 'FAB_100') then

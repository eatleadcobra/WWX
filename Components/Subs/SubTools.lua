...
function SubTools.calculateIntercept(shipPoint, subPoint, shipPosition, shipVelocity, subSpeed)
    ...
    while interceptFound == false do
        ...
        if subTimeToIntPoint < shipTravelTime then
            ...
            if speedRequired >= (3*subSpeed/4) then
    ...
    if interceptFound then
        local runInOne = Utils.VectorAdd(shipIntPoint, Utils.ScalarMult(Utils.RotateVector(shipPosition.x, 1.5708), 1100))
        local runInTwo = Utils.VectorAdd(shipIntPoint, Utils.ScalarMult(Utils.RotateVector(shipPosition.x, -1.5708), 1100))
        ...
        local attackRunEndPoint = Utils.VectorAdd(closestRunIn, Utils.ScalarMult(attackRunVector, subSpeed*(700)))
